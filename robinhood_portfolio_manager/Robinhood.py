...
class Robinhood(object):
    ...
    def generate_excel_file(self):
        ...
        self.logger.info("Excel file generated. Configure percentages for each stock.")
    ...
    def cancel_open_orders(self, side="all", sim=False):
        ...
        for order in orders:
            ...
            if (side == "all") or (side == order_side):
                ...
                self.logger.info(f"Cancelling {order_side} order of {symbol}.")
    ...
    def rebalance(self, sim=False):
        ...
        for instrument in sell_list:
            ...
            self.logger.info(f"Selling ${amount_to_sell:.2f} of {instrument}.")
            time.sleep(5)
            counter += 1
        ...
        for instrument, percent in new_investments.items():
            ...
            if instrument in current_investments:
                ...
                if amount > current_amount:
                    ...
                    self.logger.info(f"Buying ${amount_to_buy:.2f} from {instrument} to adjust to ${amount:.2f}")
                    ...
                    self.logger.info(f"Selling ${amount_to_sell:.2f} from {instrument} to adjust to ${amount:.2f}")
                else:
                    self.logger.info(f"${amount:.2f} already invested in {instrument}")
            else:
                if not sim:
                    order = rh.orders.order_buy_fractional_by_price(instrument, amount, "gfd", True)

                self.logger.info(f"Investing ${amount:.2f} in {instrument}")
